...
lu = user.shape[0]
poi1data = pd.read_csv("F:\zhudanyang\LBS1222\Flbsdataset2\\restaurant.csv", index_col=0)
#poi1data = pd.read_csv("F:\zhudanyang\LBS-FOG\FLBSdata\Health Services.csv", index_col=0)
lenp = poi1data.shape[0]


result = pd.read_csv("F:\zhudanyang\LBS1222\Flbsdataset2\\result\dimension-rq -yelp-6.csv", index_col=0)
...
def adjust_r(x, y, x1, y1, lr):
    MIN, Trange = -77.90413490766306,382.14407054013543
...
for i in range(0, 1000):
    ...
    res[i], rec[i] = query(UX1[i], UY1[i], UX[i], UY[i], PX1, PY1, PX, PY, 400)
    resemblance = sum(res) / 1000
    recall = sum(rec) / 1000

print(resemblance)
print(recall)
result.to_csv("F:\zhudanyang\LBS1222\Flbsdataset2\\result\dimension-rq -yelp-6.csv.csv")